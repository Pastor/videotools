...
bool QVideoCapture::setPosition(int pos)
{
    return m_cvCapture.set(CV_CAP_PROP_POS_FRAMES, pos);
}
...
bool QVideoCapture::open_resolutionDialog()
{
    ...
    layout.setMargin(5);
        QGroupBox groupbox;
        groupbox.setTitle("Resolution/m_framerate");
            ...
            comboboxes.setMargin(5);
                ...
                QLabel Lm_framerate("Set m_framerate:");
    ...
    //GUI CONSTRUCTION END//
        ...
        if(dialog.exec() == QDialog::Accepted)
        {
            ...
            pt_timer->setInterval((int)1000/CBm_framerate.itemText(CBm_framerate.currentIndex()).section(" ",0,0).toDouble());
            return true;
        }
    }
...
{
    return (int)m_cvCapture.get(CV_CAP_PROP_FRAME_COUNT);
}

void QVideoCapture::stepBackward()
{
    qWarning("stepBackward()");
    if(m_cvCapture.isOpened()) {
...
void QVideoCapture::speedUp()
{
    pt_timer->setInterval(pt_timer->interval() / 2);
}

void QVideoCapture::speedDown()
{
    pt_timer->setInterval(pt_timer->interval() * 2);
}
