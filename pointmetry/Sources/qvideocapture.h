...
signals:
    void frameUpdated(const cv::Mat& value); // should be emmited right after a new frame was captured, to use in your own Qt-projects first do qRegisterMetaType<cv::Mat>("cv::Mat")
    void framesInFile(int);
    //------------------------------------------
...
private:
    ...
    int m_frames;                           // stores total ammount of frames in video file
