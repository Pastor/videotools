...
void OpenDetector( // open face or feature detector from its XML file
    CvHaarClassifierCascade **cascade,  // out
    const char*            filename, // in: basename.ext of cascade
    const char*            datadir)  // in
{
    if (cascade != nullptr) {
        ...
        char path[SLEN]; sprintf(path, "%s/%s", dir, filename);
        logprintf("Open %s\n", path);
        if (((*cascade) = static_cast<CvHaarClassifierCascade *>(cvLoad(path))) == nullptr)
            Err("Cannot load %s", path);
    }
}
...
{
    CvSeq *finded;
    static const auto MAX_NFACES_IN_IMG = int(1e4); // arb, but big
    vec_Rect feats;
    ...
    auto d = static_cast<IplImage>(roi);
    finded = cvHaarDetectObjects(&d, classifier, storage, scale_factor, min_neighbors, flags, cvSize(minwidth_pixels, minwidth_pixels));
    if (finded != nullptr && finded->total > 0) {
        for (auto k = 0; k < finded->total; ++k) {
            auto rect = reinterpret_cast<CvRect *>(cvGetSeqElem(finded, k));
            feats.push_back(cv::Rect(rect->x, rect->y, rect->width, rect->height));
        }
    }
