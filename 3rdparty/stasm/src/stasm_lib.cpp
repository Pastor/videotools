...
static IplImage *gGray = nullptr;

//-----------------------------------------------------------------------------

void __declspec(dllexport) __cdecl
__Set(std::vector<stasm::DetPar>& faces, int idx)
{
    facedet_g.Set(faces, idx);
}

void
__Set(void * pFaces, int idx)
{
    auto faces = reinterpret_cast<std::vector<stasm::DetPar> *>(pFaces);
    facedet_g.Set(*faces, idx);
}

void
__Set(IplImage *pImage)
{
    if (gGray != nullptr)
        cvReleaseImage(&gGray);
    if (pImage->nChannels > 1) {
        auto size = cvSize(pImage->width, pImage->height);
        gGray = cvCreateImage(size, IPL_DEPTH_8U, 1);
        cvCvtColor(pImage, gGray, CV_BGR2GRAY);
        img_g = Image(gGray->height, gGray->width, reinterpret_cast<unsigned char*>(gGray->imageData));
    } else {
        gGray = cvCloneImage(pImage);
        img_g = Image(pImage->height, pImage->width, reinterpret_cast<unsigned char*>(pImage->imageData));
    }    
}
