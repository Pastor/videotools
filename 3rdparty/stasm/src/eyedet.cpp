...
static CvHaarClassifierCascade *pleye_det_g;  // left eye detector
static CvHaarClassifierCascade *preye_det_g;  // right eye detector
static CvHaarClassifierCascade *pmouth_det_g; // mouth detector
    ...
    const char*    datadir)    // in
{
    if (need_eyes)
    {
        ...
        OpenDetector(&pleye_det_g, "haarcascade_mcs_lefteye.xml", datadir);
        OpenDetector(&preye_det_g, "haarcascade_mcs_righteye.xml", datadir);
    }
    if (need_mouth)
    ...
    const Rect&  facerect) // in: the detected face rectangle
{
    ...
    if (left_searchrect.width)
        leyes = Detect(img, pleye_det_g, &left_searchrect,
    ...
    if (right_searchrect.width)
        reyes = Detect(img, preye_det_g, &right_searchrect,
                       EYE_SCALE_FACTOR, EYE_MIN_NEIGHBORS, EYE_DETECTOR_FLAGS,
                       facerect.width / 10);
}
